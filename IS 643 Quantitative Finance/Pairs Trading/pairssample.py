...
def buy_signal(context, data, pair_index):
    ...
    stock_x_data = data[stock_x]
    stock_x_price = math.log10(stock_x_data.price)
    shares_x = int(cash_per_trade/stock_x_data.price)
    #get stock_y current information
    stock_y_root = (context.stocks)[all_pairs[pair_index][0][1]]
    stock_y = stock_y_root[0]
    stock_y_data = data[stock_y]
    stock_y_price = math.log10(stock_y_data.price)
    shares_y = int(cash_per_trade/stock_y_data.price)
...
def handle_data(context, data):
    ...
    if hour == 16 and minute == 0:
        ...
        for ticker in ticker_list:
            ...
            stock_data = data[stock]
            stock_price = math.log10(stock_data.price)
    ...
    for pair in all_pairs:
        ...
        for n in range (0, len(trade_list[0])):
            if (trade_list[0][n] == stock_x and trade_list[1][n] == stock_y) or (trade_list[0][n] == stock_y andtrade_list[1][n] == stock_x):
                trade_exists = True
