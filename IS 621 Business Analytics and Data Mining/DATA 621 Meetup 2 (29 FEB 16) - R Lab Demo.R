# R Lab Demo: Linear Regression, Variable Selection, Ridge Regression, Lasso, PCR, PLS
# DATA 621: Business Analytics and Data Mining

# In this lab, we will use the diabetes data in Efron et al. (2003) to examine the 
# effects of ten baseline predictor variables [age, sex, body mass index (bmi), 
# average blood pressure (map), and six blood serum measurements (tc, ldl, hdl, tch, ltg, glu)] 
# on a quantitative measure of disease progression one year after baseline. 

# There are 442 diabetes patients in this data set. The data are available in the 
# R package "lars." We will employ several statistical learning techniques using the diabetes 
# data to fit linear regression, ridge regression and lasso models. We will also 
...
library(pls)
set.seed(1306)
pcr.mod <- pcr(y ~ ., data = data.train, scale = TRUE, validation = "CV")
...
mean((pcr.pred - y.test)^2) # Mean Prediction Error = 3153.626
sd((pcr.pred - y.test)^2)/sqrt(n.test) # Standard Error = 360.2942
...
set.seed(1306)
pls.mod <- plsr(y ~ ., data = data.train, scale = T, validation = "CV")
summary(pls.mod)
...
mean((pls.pred - y.test)^2) # Mean Prediction Error = 3109.772
sd((pls.pred - y.test)^2)/sqrt(n.test) # Standard Error = 355.3373
