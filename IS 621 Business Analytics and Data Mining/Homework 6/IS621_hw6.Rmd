...
I'm more interested in comparing different types of models using the same variables for this excersize. For this reason, for each classification of the AcidIndex variable, I will perform a backwards stepwise algorithm from a full model using the Akaike Information Criterion. 
...
estimates1 <- dcast(estimates1, ModelName ~ Variable, value.var = 'Value')

estimates2 <- filter(coefdiag, Measure == 'Estimate') %>%
  filter(Model %in% c('backlinmod2', 'backNBmod2', 'backpoismod2')) %>%
  select(-c(Measure, Model))

estimates2 <- dcast(estimates2, ModelName ~ Variable, value.var = 'Value')
...
SEList <- c(
  ...
  sd((wineAINum$TARGET - predict(backNBmod1))^2)/sqrt(length(wineAINum$TARGET)),
  ...
  sd((wineAICat$TARGET - predict(backNBmod2))^2)/sqrt(length(wineAICat$TARGET))
)
...
halfnorm(residuals(testmod))

summary(fullmod)

wine$TARGET_bin <- ifelse(wine$TARGET == 0,0,1)
wine$STARS_bin <- ifelse(wine$STARS == 0,0,1)

winetest1 <- na.omit(wine)
winetest1 <- select(wine, -INDEX)

fullbin <- glm(TARGET_bin ~ FixedAcidity + VolatileAcidity + CitricAcid + 
                 ResidualSugar + Chlorides + FreeSulfurDioxide + TotalSulfurDioxide + 
                 Density + pH + Sulphates + Alcohol + LabelAppeal + AcidIndex + 
                 STARS_bin, data=winetest1, family=binomial)

winetest1$bin_predict <- predict(fullbin)
...
wine$STARS <- factor(wine$STARS)
wine$AcidIndex <- factor(wine$AcidIndex)
...
p <- ggplot(wine, aes(x=FreeSulfurDioxide, y=TotalSulfurDioxide)) + stat_binhex()
