...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak of 1 is simply a Hit followed by a miss. The hit must either have 
    been preceded by a miss (otherwise it would have been part of a longer streak)
    or the first hit. A streak of 0 is defined as a miss preceded by a miss.
    (or if the first shot is a miss.)
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    His typical streak length is 0 or 1, implying that he's varying a lot between 
    hits and misses. Longer streaks are less probable, dropping off from around 25 
    for 1 length streaks to under 10 for 2 length streaks.
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    Lets count the subset of the vector simunfaircoin that's heads
...
WOAH! I got the exact expected value on the simulation I'm running now :)
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    You just need to add "prob = c(0.45,0.55)" as an argument
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    My random player ended up with a 5 hit streak, beating out Kobe's best of 4.
    The distribution appears to taper off in a more regular way, while Kobe's 
    distribution seems to have a greater number of 1-streaks. The random player 
    also has more 0-streaks than Kobe.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    If I ran the simulation of th independent shooter a second time, I'd expect
    the streak distribution to be very similar. There would be some variations 
    due to chance, but I'd expect the bar graph to look roughly the same.
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    There might be something to this! The distribution of my random player had 
    way more 0-streaks. Kobe's distribution had a lot more 1-streaks, and a lot 
    less 0-streaks. There are two potential things that might be happening: Kobe 
    might have hot hands, meaning that when he gets a hit, he's more likely to get 
    another hit. However, the fact that there are fewer 0-streaks makes me think 
    there's another potential explanation. Kobe might be more encouraged after a 
    miss. That would explain the lack of "miss streaks."
