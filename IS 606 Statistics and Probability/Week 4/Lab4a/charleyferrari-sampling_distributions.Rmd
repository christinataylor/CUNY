...
The area seems somewhat normally distributed, with a bit of a skew to the 
right (meaning that the mean is larger than the median.)
...
```
    ...
    The outliers appear to have been cut off, so the min and max are very 
    different than the min and max in the full area dataset. The mean is 
    still greater than the median, and despite the fact that the outliers 
    are gone, there is more of a positive skew. The median and mean of both 
    the sample and the population are close to eachother b ased on simply 
    eyeballing the numbers (though I imagine we'll examine this more.)
...
```
    ...
    This time, the mean is even less than before. We still have a comparable 
    skew, but once again we don't have the outliers on either side. Here, 
    my max and min seem even more clipped than in samp1.
...
There are 100 elements in this vector. Each element is the mean of a 
particular sample pulled from the population.
...
As the sample size gets larger, the center seems to get closer to 1500. The 
standard deviation of the means also seem to decrease, meaning the bell curve 
gets tighter. 
...
price_avg_mean50 <- mean(sample_means_oyo50)
...
```
    ...
    Once again, the mean of the means we collected was closer to the population 
    mean when we collected 150 samples vs the mean when we collected 50 samples.
...
sd(sample_means_oyo50)
sd(sample_means_oyo150)

```


The spread for the samples of size 50 was larger than the spread for the 
samples of size 150. Ideally, we'd like a small spread when trying to arrive 
at the population mean, so in this case, unless is was computationally 
prohibitive, we'd like to have a sample size of 150.
