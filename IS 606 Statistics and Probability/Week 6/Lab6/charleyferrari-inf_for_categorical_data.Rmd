...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    These key findings appear to be sample statistics.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    To generalize the report's findings to the global human population, we must 
    assume that the sampling method is independent, and representatitive of the 
    population as a whole. Independence is pretty simple to determine in a case 
    like this, but there could be biases in these samples. The samples might be 
    weighted towards urban areas, for example, where the population may be 
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Each row of Table 6 refers to the summary proportions from each country. 
    Each row of the atheism dataframe is a case, and represents a person who 
    replied that they were atheist or non-atheist
...
This proportion does seem to agree with the percentage in table 6. This makes 
sense, since we're looking at the raw responses.
...
the error margin for surveys of this kind is +- 3-5% at 95% confidence".
...
This relationship is a parabola. It gets larger as p approaches 0.5, and smaller 
approaching 0 at p=1 and p=0. Intuitively this makes sense, the margin of error 
is affected by the smaller of the two sample sizes we're looking at. If p is low 
or high, than there is a smaller sample size being looked at.
...
Our mean is very close to 0.1, which is what's expected.  Looking at the 
histogram, we can see a pretty tight normal distribution around 0.1
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    Lets calculate their Margin of errors:
...
```
    ...
    The margin of errors are within the given 3-5% on the survey, but this means 
    more for the Ecuador sample than for the Australian sample. 10% +- 1.8%
    isn't too bad for an estimation, but for Ecuador 2% +- 1.4%means more. 
    For a low p value, we get more variance in our answer.
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    $H_0$: There is no difference between the proportion of atheists in 2005 
    and the proportion of atheists in 2012. $p_{2005} = p_{2012}$
    
    $H_a$: There is a difference between these two proportions. 
    $p_{2005} \neq p_{2012}$
...
These two confidence intervals do indeed overlap, and we cannot reject the null 
hypothesis that there has been no change in these proportions.

    **b.** Is there convincing evidence that the United States has seen a
    change in its atheism index between 2005 and 2012?
    
    We have the same $H_0$ and $H_a$:
...
In this case, the confidence intervals don't overlap. We can reject the null 
hypothesis that the proportions between 2005 and 2012 in the US have changed.
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    ...
    Looking at a significance level of 0.05, we would expect to detect a change 
    in 5% of the countries in table 4 simply due to chance. For the 39 countries 
    listed, this comes out to 1.95, or just about 2 countries.
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    *Hint:* Refer to your plot of the relationship between $p$ and margin of 
    ...
    We have our margin of error defined as:
    
    $$ MOE = 1.96 \times \sqrt{\frac{p(1-p)}{n}} $$
    
    Technically, the p could be so small that for any n we'll end up with a 
    large margin of error. But, lets assume that the lowest proportion we have 
    (either p or 1-p) is 1%, meaning that the proportion of atheists is between 
    1% and 99%. In this case, lets solve for n with a MOE of .01, when p = .01:
    
    $$ .01 = 1.96 \times \sqrt{\frac{0.01*0.99}{n}} $$
