...
These both seem pretty evenly distributed. The medians are close to the means, indicating low 
levels of skewness. 
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    ...
    Yes, based no the plot above, this is very close to a normal distribution.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    The normal probability plot does look similar to the plots created for the simulated
    data. I'm a bit concerned about the higher and lower quantiles, however, which seem to
    deviate a bit. But, in the simulations, there appears to be more deviation there as 
    well. I imagine because there are fewer datapoints at the extremes, tiny deviations 
    can create what look like significant changes.
...
```
    ...
    For weights, the data appears to be less normally distributed. There is still more 
    variation in the tails, but this time it's greater in the data than it is in the other 
    simulations.
...
What is the probability that a randomly chosen female is shorter than 5'5? That's
about 165cm.
...
What is the probability that a randomly chosen female is between 100 and 125 pounds?
(that's 45.3kg and 56.7kg)

```{r}

pnorm(q=56.7, mean = mean(fdims$wgt), sd = sd(fdims$wgt)) - 
  pnorm(q=45.3, mean = mean(fdims$wgt), sd = sd(fdims$wgt))

sum(fdims$wgt > 45.3 & fdims$wgt < 56.7) / length(fdims$wgt)
...
answers(fdims$bii.di)
answers(fdims$elb.di)
answers(bdims$age)
answers(fdims$che.de)
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    This could be due to rounding of measurements. For age, this is certainly 
    rounded to the nearest year, while elbow diameter could have been rounded 
    to the nearest cm, or mm.
...
answers(fdims$kne.di)

```

From the qq plot this appears to be right-skewed, and the histogram corroborates this.
