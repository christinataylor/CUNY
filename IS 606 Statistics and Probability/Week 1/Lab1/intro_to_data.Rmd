...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    Peaking at the number of observations, it would appear there are 20000 cases, 
    and 9 variables. 
...
onequ <- summary(cdc$height)[[2]]
threequ <- summary(cdc$height)[[4]]

#########################

threequ - onequ

onequ <- summary(cdc$age)[[2]]
threequ <- summary(cdc$height)[[4]]
...
# what proportion report being in excellent health?

table(cdc$exerany)[2]/20000
...
The mosaic plot reveals that a higher proportion of males have smoked 100 cigarettes when compared 
to women.
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    This box plot groups the sample by general health, and then shows each group's distribution of 
    BMI. This allows us to get an idea of how these two variables are related. This is very similar 
    to a scatter plot with two numerical variables.
    
    Lets see how exercise affects people's BMI. 
...
```
    ...
    Interesting! Looking at the IQR, it would appear that people who have exercised in the past 
    month have lower BMIs, but there seem to be more outliers in the group who have exercised in 
    the past month. I wonder what that means.
...
```
    ...
    There appear to be outliers (surprisingly, there appear to be a few very high 
    desired weights in the survey...) Lets try to remove them.
    
```{r}
library(dplyr)

ggplot(filter(cdc, wtdesire<400 & weight < 300), aes(x=weight, y=wtdesire)) + geom_point()

```
    
    Looking at this, there appears to be less of a linear relationship than I 
    originally thought. There are a lot of people at their desired weight (which is
    represented by the diagonal line that seems to appear). More people are below 
    the line than above, meaning they'd like to lose weight.
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    wdiff is a numeric variable. If it's 0, that means the person is at their 
    desired weight. Positive numbers mean the person would like to gain weight, 
    and negative numbers mean the person would like to lose weight.
...
```
    ...
    This tells us most people want to lose weight. The median person in the sample 
    wants to lose 10 pounds. Lots of people are closer to this median, while there 
    are some outliers on both sides.
...
# To show my summaries, I'm going to add wdiff as an extra variable in the data frame.

cdc$wdiff <- cdc$wtdesire - cdc$weight

summary(filter(cdc, gender=="m")$wdiff)

summary(filter(cdc, gender=="f")$wdiff)

```

    It's difficult to see the differences on the boxplot, but the numerical summary 
    gives us an idea of what's going on. Looking at the median, males want to lose 
    less weight than famales (5lb vs 10lb). Females have a slightly more negative 
    IQR.
...
dim(filter(cdc, weight<meanw+sdw & weight>meanw-sdw))[1]/dim(cdc)[1]

```

    I prefer using dplyr to subset my data frames! I just took the dimensions of the 
    filtered data frame, and took it's proportion from the total number of cases.
